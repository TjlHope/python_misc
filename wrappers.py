...
def s(i):
    """Function for intelligent string conversion within iterables."""
...
def header_join(iterable, header_sep, sep=None):
    """Joins an iterable with a different string between the first elements."""
    if sep is None:
        sep = header_sep
    return header_sep.join((s(iterable[0]), sep.join(s(iterable[1:]))))
...
def display_TraceBack(logger=logging.getLogger('TB'), level='error'):
    ...
    while exc_tb:
        ...
        log(''.join((file_name, " - '", function_name, "' - ", str(line_no))))
